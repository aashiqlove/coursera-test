...
if(box==".div1"||box==".div7"||box==".div9"||box==".div10"){
	pr.style.display="block";
	ne.style.display="block";
	if(box==".div10"){
		var parser = new DOMParser();
        var doc = parser.parseFromString(div10, 'text/html');
		
		const cro =doc.querySelectorAll(".bank");
		console.log(cro.length);
		if (cro.length > 1) {
		    pr.style.display="block";
		    ne.style.display="block";
		}else{
...
if(box==".div0"){
	...
	var syn =`<h1>இயந்திரங்கள்</h1>
	<br> <font size="3"> உங்கள் உள்ளூர் வேளாண் கருவி அலுவலகத்தைத் தொடர்பு கொள்ளவும் </font> <br>  044 - 29510922 , 044 - 29510822 , 044 - 29515322 </div>
	<div class="in_content">${div9}</div>`;
	content.innerHTML=syn;
}
else if(box==".div10"){
	var syn =`<h1>கிடங்கு</h1>
	<div class="in_content">${div10}</div>`;
	content.innerHTML=syn;
}
}
...
function boxarea_default(){
    ...
    .then(html => {
        ...
        const basicDetailsDiv9 = tempContainer.querySelector('.div9');
        const basicDetailsDiv10 = tempContainer.querySelector('.div10');
      ...
      div9=basicDetailsDiv9.outerHTML;
      div10=basicDetailsDiv10.outerHTML;
